...
{
    [Fact]
    public void UpdatesAreGroupedByDependencyNameAndVersion()
    {
        ...
        var updateOperations = GroupUpdateAllVersionsHandler.GroupUpdateOperationsByDependency(discovery);
        var updateOperation = Assert.Single(updateOperations);
        Assert.Equal("some.dependency/1.0.0", updateOperation.Key.ToString());
        var operationProjects = updateOperation.ToArray();
        AssertEx.Equal(["/project1.csproj", "/project2.csproj"], operationProjects.Select(p => p.ProjectPath).ToArray());
    }
